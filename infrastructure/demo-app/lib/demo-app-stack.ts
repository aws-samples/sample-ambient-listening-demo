...
export class DemoAppStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: DemoAppStackProps) {
    ...
    const regionCondition = new cdk.CfnCondition(this, 'UnsupportedRegionCondition', {
    ...
    new cdk.CfnRule(this, 'RegionValidationRule', {
      assertions: [
        {
          assert: cdk.Fn.conditionNot(regionCondition),
    ...
    // Private subnets with egress: ECS Fargate tasks (NAT Gateway for outbound)
    // Private isolated subnets: Aurora, ElastiCache, EFS (no internet access needed)
    ...
    // NOTE: Connect Health actions do not support resource-level permissions — resource '*' is
    // required per AWS documentation. This is the only acceptable use of wildcard resources.
    ...
    // KMS decrypt permission scoped to the output bucket key (needed to read encrypted S3 objects)
    this.ecsTaskRole.addToPolicy(new iam.PolicyStatement({
      sid: 'KmsDecryptForS3Objects',
      effect: iam.Effect.ALLOW,
      actions: ['kms:Decrypt'],
      resources: [this.outputBucketKey.keyArn],
    }));

    // Secrets Manager read access scoped to the three specific secret ARNs
    this.ecsTaskRole.addToPolicy(new iam.PolicyStatement({
    ...
    // Domain name for Connect Health (used as environment variable)
    const connectHealthDomainName = `${id}-ambient-domain`;

    this.taskDefinition.addContainer('DemoAppContainer', {
      image: ecs.ContainerImage.fromRegistry('public.ecr.aws/docker/library/node:20-slim'),
    ...
    // Target group pointing to ECS service on port 3000
    ...
    // This is expected and required for an internet-facing ALB architecture.
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      [
    ...
    // This is the standard CDK pattern for least-privilege policies scoped to specific resources.
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      [
      ...
      [
        {
          ...
          reason: 'CDK generates inline policies for roles using addToPolicy(). These policies are scoped to specific resource ARNs and follow least-privilege principles. Converting to managed policies would reduce maintainability without improving security posture.',
        },
      ]
    );

    // VPC default security group — CDK does not provide a direct way to modify the default
    // security group rules. A custom resource would be needed for production deployments.
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      ...
      [
        {
          ...
          reason: 'CloudFormation does not support modifying the default security group rules during VPC creation. All workloads use dedicated security groups with restricted ingress. The default security group is not referenced by any resource. Production deployments should use a custom resource to close the default security group.',
        },
      ]
    );
    ...
    // VPC Flow Log log group encryption suppression
    NagSuppressions.addResourceSuppressionsByPath(
      this,
      ...
      [
        {
          ...
          reason: 'VPC Flow Log group is auto-created by CDK and does not support encryptionKey in the FlowLog construct. The flow logs contain network metadata (not PHI). Production deployments should use a custom log group with KMS encryption.',
        },
      ]
    );
